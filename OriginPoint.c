...
enum line_code{END_OF_BUFFER=-2, EMPTY_LINE=-1};
char* path_default={"t1.txt"};
char* path_save_default={"j1.txt"};
char path[40], path_save[40];
...
void ask_for_file(char mode)
{
	switch(mode)
	{
		...
		case 'w':
			...
			scanf("%s", path_save);
			if(path_save[0]=='d')
				strcpy(path_save, path_save_default);
			break;
	}
	fflush(stdin);//żeby mieć pewność, że program się zatrzyma w razie błędu, aż nie dostanie inputu użytkownika
}

int get_bite(FILE* desc, char buffer[BUF_SIZE])
{
	char temp_char;
	int count=0;
	memset(buffer, 0, strlen(buffer));
	while((temp_char=fgetc(desc))!=EOF&&count<(BUF_SIZE-1))
	{
		if(temp_char!='\r'&&temp_char!='\n'&&temp_char!='\t')//ignorowanie znaków nowej linii i tabulatorów
		{
	...
	buffer[count]='\0';//jako że kopiuje ręcznie znaki, to zaznaczyć koniec string też trzeba ręcznie
	ungetc(temp_char,desc);//zwrócenie nadmiarowego znaku z getc
	return count;//wzorem UNIXowej funkcji read() zwrócenie ilości zaczytanych znaków
}
...
int main()
{
	...
	FILE* desc;
	desc=fopen(path, "r");//otwarcie pliku źródłowego
	if(desc==NULL)
	{
